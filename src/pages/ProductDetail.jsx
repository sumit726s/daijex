...
const ProductDetail = () => {
 ...
 useEffect(() => {
    const fetchProduct = async () => {
      try {
        setLoading(true);
        // Assuming you have an error state initialized like in the Page component
        if (typeof setError === 'function') setError(null); 
        
        // 1. Fetch ALL accessories from Drupal (No title filter!)
        // This endpoint should already have your ?include=... parameters for images
        const response = await api.get(ENDPOINTS.accessories);
        const allAccessories = response.data.data;
        const includedData = response.data.included; // Keep this for relationships!

        // 2. Use vanilla JavaScript to find the matching alias
        const currentPath = location.pathname; // e.g., "/accessory/front-bumper"
        const matchedProduct = allAccessories.find(
          (item) => item.attributes?.path?.alias === currentPath
        );

        if (matchedProduct) {
          // Pass the single matched item AND the full included array to your formatter
          setProduct(formatAccessoryData(matchedProduct, includedData));
        } else {
          if (typeof setError === 'function') setError('Product not found');
        }
      } catch (err) {
        console.error("Product Detail Sync Error:", err);
        if (typeof setError === 'function') setError('Failed to load product.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <Layout>
      <SEO 
        ...
        fallbackTitle={`${product.title} | Daijex Auto Accessories`} 
      />
      <div className="py-8 w-full mx-auto px-6">
